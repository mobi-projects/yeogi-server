...
class DefaultPostReadRepositoryTest {
    ...
    void higherViewCountRecordAlwaysComesFirst() {
        ...
        assertThat(expected.get().getId()).isEqualTo(post.getId());
    }
    ...
    void ifSameViewCountThenOrderByCreatedAtDesc() {
        ...
        assertThat(expected.isPresent()).isTrue();
        assertThat(post2.getCreatedAt().isAfter(post.getCreatedAt())).isTrue();
        assertThat(expected.get().getId()).isEqualTo(post2.getId());
    }
